...
public class ListSingleSendStat extends ApiKeyBase {
    ...
    public ApiResponse<SinglesendsResponse> send(
        ...
        if (response == null) {
            throw new ApiConnectionException(
                "ListSingleSendStat creation failed: Unable to connect to server"
            );
        } else if (
        ...
        return new ApiResponse(
            statusCode,
            JsonUtil.fromJson(response.getStream(), SinglesendsResponse.class),
            response.getHeaders()
        );
    }

    private void addQueryParams(Request request) {
        if (singlesendIds != null) {
            request.addQueryParam("singlesend_ids", singlesendIds.toString());
        }
        if (pageSize != null) {
