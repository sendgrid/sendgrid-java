...
import com.sendgrid.rest.api.v3.ips.models.PutIpsPoolsPoolName404Response;
import com.sendgrid.rest.api.v3.ips.models.PutIpsPoolsPoolNameRequest;
import com.sendgrid.util.JsonUtil;
import com.sendgrid.util.Matcher;
import lombok.RequiredArgsConstructor;

@RequiredArgsConstructor
public class PutIpPoolById {
    private final String poolName;
    private final PutIpsPoolsPoolNameRequest putIpsPoolsPoolNameRequest;


    public IpPools200 send(final TokenRestClient client) {
        ...
        if (response == null) {
            throw new ApiConnectionException(
                    "PutIpPoolById creation failed: Unable to connect to server"
            );
        } else if (!TokenRestClient.SUCCESS.test(response.getStatusCode())) {

            int statusCode = response.getStatusCode();
            if (Matcher.matches(Integer.toString(statusCode), "404")) {
                PutIpsPoolsPoolName404Response error = JsonUtil.fromJson(
                        response.getStream(),
                        PutIpsPoolsPoolName404Response.class
                );
                throw new ApiErrorResponse(statusCode, null, error, response.getHeaders());
            }

        }
        return JsonUtil.fromJson(response.getStream(), IpPools200.class);
    }


    private void body(final Request.Builder builder) {
        if (putIpsPoolsPoolNameRequest != null) {
            builder.body(JsonUtil.toJson(putIpsPoolsPoolNameRequest));
        }
    }
