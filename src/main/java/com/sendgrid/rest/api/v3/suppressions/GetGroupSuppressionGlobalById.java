...
import com.sendgrid.rest.api.v3.suppressions.models.RetrieveAGlobalSuppressionResponse;
import com.sendgrid.util.JsonUtil;
import lombok.RequiredArgsConstructor;

@RequiredArgsConstructor
public class GetGroupSuppressionGlobalById {
    private final String email;
    private String onBehalfOf;


    public RetrieveAGlobalSuppressionResponse send(final TokenRestClient client) {
        String path = "/v3/asm/suppressions/global/{email}";
        Request.Builder builder = new Request.Builder(HttpMethod.GET, path)
                .addPathParam("email", email.toString());
        ...
        if (response == null) {
            throw new ApiConnectionException(
                    "GetGroupSuppressionGlobalById creation failed: Unable to connect to server"
            );
        } else if (!TokenRestClient.SUCCESS.test(response.getStatusCode())) {

            int statusCode = response.getStatusCode();
        }
        return JsonUtil.fromJson(response.getStream(), RetrieveAGlobalSuppressionResponse.class);
    }
