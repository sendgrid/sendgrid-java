...
public class SearchRecipient extends ApiKeyBase {

    @Setter
    private SearchRecipientRequest searchRecipientRequest;

    public ApiResponse<SearchRecipient200Response> send(
        ...
        if (response == null) {
            throw new ApiConnectionException(
                "SearchRecipient creation failed: Unable to connect to server"
            );
        } else if (
    ...
    private void addBody(final Request request) {
        if (searchRecipientRequest != null) {
            request.addBody(JsonUtil.toJson(searchRecipientRequest));
        }
    }
}
