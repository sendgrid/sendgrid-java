...
import com.sendgrid.rest.api.v3.suppressions.models.GetAsmSuppressions200ResponseInner;
import com.sendgrid.util.JsonUtil;
import lombok.RequiredArgsConstructor;

@RequiredArgsConstructor
public class GetSuppressions {
    private String onBehalfOf;


    public List<GetAsmSuppressions200ResponseInner> send(final TokenRestClient client) {
        ...
        if (response == null) {
            throw new ApiConnectionException(
                    "GetSuppressions creation failed: Unable to connect to server"
            );
        } else if (!TokenRestClient.SUCCESS.test(response.getStatusCode())) {

            int statusCode = response.getStatusCode();
        }
        return JsonUtil.fromJson(response.getStream(), List & lt; GetAsmSuppressions200ResponseInner & gt;.class);
    }
