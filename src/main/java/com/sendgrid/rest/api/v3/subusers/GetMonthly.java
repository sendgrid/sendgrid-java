...
public class GetMonthly {
    private Integer limit;
    private Integer offset;
    private String query;


    public SubuserStats send(final TokenRestClient client) {
        ...
        if (response == null) {
            throw new ApiConnectionException(
                    "GetMonthly creation failed: Unable to connect to server"
            );
        } else if (!TokenRestClient.SUCCESS.test(response.getStatusCode())) {

            int statusCode = response.getStatusCode();
        }
        return JsonUtil.fromJson(response.getStream(), SubuserStats.class);
    }
