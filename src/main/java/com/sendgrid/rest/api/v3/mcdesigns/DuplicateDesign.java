...
import com.sendgrid.rest.api.v3.mcdesigns.models.ApiError;
import com.sendgrid.rest.api.v3.mcdesigns.models.ApiErrors;
import com.sendgrid.rest.api.v3.mcdesigns.models.DesignDuplicateInput;
...
public class DuplicateDesign extends ApiKeyBase {

    private final UUID id;

    @Setter
    private DesignDuplicateInput designDuplicateInput;
    ...
    public ApiResponse<DesignOutput> send(final ApiKeyRestClient client) {
        ...
        if (response == null) {
            throw new ApiConnectionException(
                "DuplicateDesign creation failed: Unable to connect to server"
            );
        } else if (
            ...
            if (Matcher.matches(Integer.toString(statusCode), "400")) {
                ApiError error = JsonUtil.fromJson(
                    response.getStream(),
                    ApiError.class
                );
    ...
    private void addBody(final Request request) {
        if (designDuplicateInput != null) {
            request.addBody(JsonUtil.toJson(designDuplicateInput));
        }
    }
}
