...
public class GetIntegrationCertificate {
    private final String integrationId;


    public List<SsoCertificateBody> send(final TokenRestClient client) {
        String path = "/v3/sso/integrations/{integration_id}/certificates";
        Request.Builder builder = new Request.Builder(HttpMethod.GET, path)
                .addPathParam("integration_id", integrationId.toString());
        Request request = builder.build();
        Response response = client.request(request);

        if (response == null) {
            throw new ApiConnectionException(
                    "GetIntegrationCertificate creation failed: Unable to connect to server"
            );
        ...
        return JsonUtil.fromJson(response.getStream(), List & lt; SsoCertificateBody & gt;.class);
    }
