...
import com.sendgrid.rest.api.v3.mclists.models.DeleteMarketingListsId404Response;
import com.sendgrid.util.JsonUtil;
import com.sendgrid.util.Matcher;
import lombok.RequiredArgsConstructor;

@RequiredArgsConstructor
public class DeleteListsById {
    private final String id;
    private String query;


    public String send(final TokenRestClient client) {
        ...
        if (response == null) {
            throw new ApiConnectionException(
                    "DeleteListsById creation failed: Unable to connect to server"
            );
        } else if (!TokenRestClient.SUCCESS.test(response.getStatusCode())) {

            int statusCode = response.getStatusCode();
            if (Matcher.matches(Integer.toString(statusCode), "404")) {
                DeleteMarketingListsId404Response error = JsonUtil.fromJson(
                        response.getStream(),
                        DeleteMarketingListsId404Response.class
                );
                throw new ApiErrorResponse(statusCode, null, error, response.getHeaders());
            }

        }
        return JsonUtil.fromJson(response.getStream(), String.class);
    }
