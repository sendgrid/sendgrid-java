...
import com.sendgrid.rest.api.v3.integrations.models.AddIntegration400Response;
...
import com.sendgrid.rest.api.v3.integrations.models.IntegrationPatch;
...
public class UpdateIntegration extends ApiKeyBase {

    private final String id;

    @Setter
    private IntegrationPatch integrationPatch;

    public ApiResponse<Integration> send(final ApiKeyRestClient client) {
        ...
        if (response == null) {
            throw new ApiConnectionException(
                "UpdateIntegration creation failed: Unable to connect to server"
            );
        } else if (
            ...
            if (Matcher.matches(Integer.toString(statusCode), "400")) {
                AddIntegration400Response error = JsonUtil.fromJson(
                    response.getStream(),
                    AddIntegration400Response.class
                );
        ...
        return new ApiResponse(
            statusCode,
            JsonUtil.fromJson(response.getStream(), Integration.class),
            response.getHeaders()
        );
    }
    ...
    private void addBody(final Request request) {
        if (integrationPatch != null) {
            request.addBody(JsonUtil.toJson(integrationPatch));
        }
    }
}
