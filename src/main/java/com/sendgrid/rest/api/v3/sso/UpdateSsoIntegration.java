...
public class UpdateSsoIntegration extends ApiKeyBase {
    ...
    public ApiResponse<SsoIntegration> send(final ApiKeyRestClient client) {
        ...
        if (response == null) {
            throw new ApiConnectionException(
                "UpdateSsoIntegration creation failed: Unable to connect to server"
            );
        } else if (
        ...
        return new ApiResponse(
            statusCode,
            JsonUtil.fromJson(response.getStream(), SsoIntegration.class),
            response.getHeaders()
        );
    }
    ...
    private void addQueryParams(Request request) {
        if (si != null) {
            request.addQueryParam("si", si.toString());
        }
    }

    private void addBody(final Request request) {
        if (postPatchIntegrationRequest != null) {
            request.addBody(JsonUtil.toJson(postPatchIntegrationRequest));
        }
    }
}
