...
import com.sendgrid.rest.api.v3.ips.models.GetIpsAssigned200ResponseInner;
import com.sendgrid.util.JsonUtil;
import lombok.RequiredArgsConstructor;

@RequiredArgsConstructor
public class GetIpAssigned {


    public List<GetIpsAssigned200ResponseInner> send(final TokenRestClient client) {
        ...
        if (response == null) {
            throw new ApiConnectionException(
                    "GetIpAssigned creation failed: Unable to connect to server"
            );
        } else if (!TokenRestClient.SUCCESS.test(response.getStatusCode())) {

            int statusCode = response.getStatusCode();
        }
        return JsonUtil.fromJson(response.getStream(), List & lt; GetIpsAssigned200ResponseInner & gt;.class);
    }
