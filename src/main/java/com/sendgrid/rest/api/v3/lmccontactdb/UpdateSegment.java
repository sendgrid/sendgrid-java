...
import com.sendgrid.rest.api.v3.lmccontactdb.models.UpdateSegmentRequest;
...
public class UpdateSegment extends ApiKeyBase {
    ...
    @Setter
    private UpdateSegmentRequest updateSegmentRequest;
    ...
    public ApiResponse<ContactdbSegments> send(final ApiKeyRestClient client) {
        ...
        if (response == null) {
            throw new ApiConnectionException(
                "UpdateSegment creation failed: Unable to connect to server"
            );
        } else if (
        ...
        return new ApiResponse(
            statusCode,
            JsonUtil.fromJson(response.getStream(), ContactdbSegments.class),
            response.getHeaders()
        );
    }
    ...
    private void addBody(final Request request) {
        if (updateSegmentRequest != null) {
            request.addBody(JsonUtil.toJson(updateSegmentRequest));
        }
    }
}
