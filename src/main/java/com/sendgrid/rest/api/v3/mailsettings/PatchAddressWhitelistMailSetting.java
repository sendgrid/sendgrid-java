...
import com.sendgrid.rest.api.v3.mailsettings.models.PatchMailSettingsAddressWhitelistRequest;
import com.sendgrid.util.JsonUtil;
import lombok.RequiredArgsConstructor;

@RequiredArgsConstructor
public class PatchAddressWhitelistMailSetting {
    private String onBehalfOf;
    private final PatchMailSettingsAddressWhitelistRequest patchMailSettingsAddressWhitelistRequest;


    public MailSettingsAddressWhitelabel200 send(final TokenRestClient client) {
        ...
        if (response == null) {
            throw new ApiConnectionException(
                    "PatchAddressWhitelistMailSetting creation failed: Unable to connect to server"
            );
        } else if (!TokenRestClient.SUCCESS.test(response.getStatusCode())) {

            int statusCode = response.getStatusCode();
        }
        return JsonUtil.fromJson(response.getStream(), MailSettingsAddressWhitelabel200.class);
    }
    ...
    private void body(final Request.Builder builder) {
        if (patchMailSettingsAddressWhitelistRequest != null) {
            builder.body(JsonUtil.toJson(patchMailSettingsAddressWhitelistRequest));
        }
    }
