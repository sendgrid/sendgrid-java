...
import com.sendgrid.rest.api.v3.verifiedsenders.models.PostVerifiedSenders400Response;
...
public class PatchVerifiedSendersById {
    private final String id;
    private final VerifiedSenderRequest verifiedSenderRequest;


    public VerifiedSenderResponse send(final TokenRestClient client) {
        ...
        if (response == null) {
            throw new ApiConnectionException(
                    "PatchVerifiedSendersById creation failed: Unable to connect to server"
            );
        } else if (!TokenRestClient.SUCCESS.test(response.getStatusCode())) {

            int statusCode = response.getStatusCode();
            if (Matcher.matches(Integer.toString(statusCode), "400")) {
                PostVerifiedSenders400Response error = JsonUtil.fromJson(
                        response.getStream(),
                        PostVerifiedSenders400Response.class
                );
        ...
        return JsonUtil.fromJson(response.getStream(), VerifiedSenderResponse.class);
    }


    private void body(final Request.Builder builder) {
        if (verifiedSenderRequest != null) {
            builder.body(JsonUtil.toJson(verifiedSenderRequest));
        }
    }
