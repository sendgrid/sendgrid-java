...
import com.sendgrid.rest.api.v3.lmccampaigns.models.Campaigns2xx;
import com.sendgrid.rest.api.v3.lmccampaigns.models.ErrorResponse;
import com.sendgrid.rest.api.v3.lmccampaigns.models.PostCampaignsRequest;
...
public class CreateCampaign extends ApiKeyBase {

    @Setter
    private String onBehalfOf;

    @Setter
    private PostCampaignsRequest postCampaignsRequest;

    public ApiResponse<Campaigns2xx> send(final ApiKeyRestClient client) {
        ...
        if (response == null) {
            throw new ApiConnectionException(
                "CreateCampaign creation failed: Unable to connect to server"
            );
        } else if (
        ...
        return new ApiResponse(
            statusCode,
            JsonUtil.fromJson(response.getStream(), Campaigns2xx.class),
            response.getHeaders()
        );
    }
    ...
    private void addBody(final Request request) {
        if (postCampaignsRequest != null) {
            request.addBody(JsonUtil.toJson(postCampaignsRequest));
        }
    }
}
