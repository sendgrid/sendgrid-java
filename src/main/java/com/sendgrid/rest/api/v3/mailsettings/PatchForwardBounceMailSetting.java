...
import com.sendgrid.rest.api.v3.mailsettings.models.MailSettingsForwardBounce;
import com.sendgrid.util.JsonUtil;
import lombok.RequiredArgsConstructor;

@RequiredArgsConstructor
public class PatchForwardBounceMailSetting {
    private String onBehalfOf;
    private final MailSettingsForwardBounce mailSettingsForwardBounce;


    public MailSettingsForwardBounce send(final TokenRestClient client) {
        String path = "/v3/mail_settings/forward_bounce";
        ...
        if (response == null) {
            throw new ApiConnectionException(
                    "PatchForwardBounceMailSetting creation failed: Unable to connect to server"
            );
        } else if (!TokenRestClient.SUCCESS.test(response.getStatusCode())) {

            int statusCode = response.getStatusCode();
        }
        return JsonUtil.fromJson(response.getStream(), MailSettingsForwardBounce.class);
    }
    ...
    private void body(final Request.Builder builder) {
        if (mailSettingsForwardBounce != null) {
            builder.body(JsonUtil.toJson(mailSettingsForwardBounce));
        }
    }
