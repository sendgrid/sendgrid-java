...
import com.sendgrid.rest.api.v3.linkbranding.models.AssociateBrandedLinkWithSubuserRequest;
...
public class AssociateBrandedLinkWithSubuser extends ApiKeyBase {

    private final Integer linkId;

    @Setter
    private AssociateBrandedLinkWithSubuserRequest associateBrandedLinkWithSubuserRequest;
    ...
    public ApiResponse<LinkBranding200> send(final ApiKeyRestClient client) {
        ...
        if (response == null) {
            throw new ApiConnectionException(
                "AssociateBrandedLinkWithSubuser creation failed: Unable to connect to server"
            );
        } else if (
    ...
    private void addPathParams(Request request) {
        if (linkId != null) {
            request.addPathParam("link_id", linkId.toString());
        }
    }

    private void addBody(final Request request) {
        if (associateBrandedLinkWithSubuserRequest != null) {
            request.addBody(
                JsonUtil.toJson(associateBrandedLinkWithSubuserRequest)
            );
        }
    }
}
