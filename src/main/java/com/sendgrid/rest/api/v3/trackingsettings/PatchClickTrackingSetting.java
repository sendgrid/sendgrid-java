...
import com.sendgrid.rest.api.v3.trackingsettings.models.PatchTrackingSettingsClickRequest;
import com.sendgrid.util.JsonUtil;
import lombok.RequiredArgsConstructor;

@RequiredArgsConstructor
public class PatchClickTrackingSetting {
    private String onBehalfOf;
    private final PatchTrackingSettingsClickRequest patchTrackingSettingsClickRequest;


    public ClickTracking send(final TokenRestClient client) {
        ...
        if (response == null) {
            throw new ApiConnectionException(
                    "PatchClickTrackingSetting creation failed: Unable to connect to server"
            );
        } else if (!TokenRestClient.SUCCESS.test(response.getStatusCode())) {

            int statusCode = response.getStatusCode();
        }
        return JsonUtil.fromJson(response.getStream(), ClickTracking.class);
    }
    ...
    private void body(final Request.Builder builder) {
        if (patchTrackingSettingsClickRequest != null) {
            builder.body(JsonUtil.toJson(patchTrackingSettingsClickRequest));
        }
    }
