...
public class GetMessage extends ApiKeyBase {

    private final String msgId;

    public ApiResponse<Message> send(final ApiKeyRestClient client) {
        ...
        if (response == null) {
            throw new ApiConnectionException(
                "GetMessage creation failed: Unable to connect to server"
            );
        } else if (
            ...
            if (Matcher.matches(Integer.toString(statusCode), "404")) {
                GetMessage404Response error = JsonUtil.fromJson(
                    response.getStream(),
                    GetMessage404Response.class
                );
                throw new ApiErrorResponse(
                    statusCode,
                    null,
        ...
        return new ApiResponse(
            statusCode,
            JsonUtil.fromJson(response.getStream(), Message.class),
            response.getHeaders()
        );
    }
    ...
    private void addPathParams(Request request) {
        if (msgId != null) {
            request.addPathParam("msg_id", msgId.toString());
        }
    }
}
