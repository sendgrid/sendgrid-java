...
public class PostRefreshById {
    private final UUID segmentId;
    private final SegmentRefreshRequest segmentRefreshRequest;


    public SegmentRefresh202 send(final TokenRestClient client) {
        ...
        if (response == null) {
            throw new ApiConnectionException(
                    "PostRefreshById creation failed: Unable to connect to server"
            );
            ...
            if (Matcher.matches(Integer.toString(statusCode), "404")) {
                SegmentError error = JsonUtil.fromJson(
                        response.getStream(),
                        SegmentError.class
                );
                throw new ApiErrorResponse(statusCode, null, error, response.getHeaders());
        ...
        return JsonUtil.fromJson(response.getStream(), SegmentRefresh202.class);
    }


    private void body(final Request.Builder builder) {
        if (segmentRefreshRequest != null) {
            builder.body(JsonUtil.toJson(segmentRefreshRequest));
        }
    }
