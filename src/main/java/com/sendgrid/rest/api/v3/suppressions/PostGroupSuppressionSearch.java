...
public class PostGroupSuppressionSearch {
    private final String groupId;
    private String onBehalfOf;
    private final SuppressionsRequest suppressionsRequest;


    public List<String> send(final TokenRestClient client) {
        String path = "/v3/asm/groups/{group_id}/suppressions/search";
        ...
        if (response == null) {
            throw new ApiConnectionException(
                    "PostGroupSuppressionSearch creation failed: Unable to connect to server"
            );
        } else if (!TokenRestClient.SUCCESS.test(response.getStatusCode())) {

            int statusCode = response.getStatusCode();
        }
        return JsonUtil.fromJson(response.getStream(), List & lt; String & gt;.class);
    }
