...
import com.sendgrid.rest.api.v3.mcsinglesends.models.ScheduleSingleSend201Response;
import com.sendgrid.rest.api.v3.mcsinglesends.models.ScheduleSingleSendRequest;
...
public class ScheduleSingleSend extends ApiKeyBase {

    private final String id;

    @Setter
    private ScheduleSingleSendRequest scheduleSingleSendRequest;

    public ApiResponse<ScheduleSingleSend201Response> send(
        ...
        if (response == null) {
            throw new ApiConnectionException(
                "ScheduleSingleSend creation failed: Unable to connect to server"
            );
        } else if (
        ...
        return new ApiResponse(
            ...
            JsonUtil.fromJson(
                response.getStream(),
                ScheduleSingleSend201Response.class
            ),
            response.getHeaders()
        );
    }
    ...
    private void addBody(final Request request) {
        if (scheduleSingleSendRequest != null) {
            request.addBody(JsonUtil.toJson(scheduleSingleSendRequest));
        }
    }
}
