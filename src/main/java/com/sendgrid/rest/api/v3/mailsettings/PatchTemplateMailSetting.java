...
import com.sendgrid.rest.api.v3.mailsettings.models.PatchMailSettingsTemplate200Response;
import com.sendgrid.rest.api.v3.mailsettings.models.PatchMailSettingsTemplateRequest;
import com.sendgrid.util.JsonUtil;
import lombok.RequiredArgsConstructor;

@RequiredArgsConstructor
public class PatchTemplateMailSetting {
    private String onBehalfOf;
    private final PatchMailSettingsTemplateRequest patchMailSettingsTemplateRequest;


    public PatchMailSettingsTemplate200Response send(final TokenRestClient client) {
        ...
        if (response == null) {
            throw new ApiConnectionException(
                    "PatchTemplateMailSetting creation failed: Unable to connect to server"
            );
        } else if (!TokenRestClient.SUCCESS.test(response.getStatusCode())) {

            int statusCode = response.getStatusCode();
        }
        return JsonUtil.fromJson(response.getStream(), PatchMailSettingsTemplate200Response.class);
    }
    ...
    private void body(final Request.Builder builder) {
        if (patchMailSettingsTemplateRequest != null) {
            builder.body(JsonUtil.toJson(patchMailSettingsTemplateRequest));
        }
    }
