...
public class GetSuppressionInvalidEmailById {
    private final String email;
    private String onBehalfOf;


    public List<InvalidEmail> send(final TokenRestClient client) {
        String path = "/v3/suppression/invalid_emails/{email}";
        ...
        if (response == null) {
            throw new ApiConnectionException(
                    "GetSuppressionInvalidEmailById creation failed: Unable to connect to server"
            );
        } else if (!TokenRestClient.SUCCESS.test(response.getStatusCode())) {

            int statusCode = response.getStatusCode();
        }
        return JsonUtil.fromJson(response.getStream(), List & lt; InvalidEmail & gt;.class);
    }
