...
import com.sendgrid.rest.api.v3.suppressions.models.SuppressionGroupRequestBaseProps;
import com.sendgrid.util.JsonUtil;
import lombok.RequiredArgsConstructor;

@RequiredArgsConstructor
public class PatchGroupsById {
    private final String groupId;
    private String onBehalfOf;
    private final SuppressionGroupRequestBaseProps suppressionGroupRequestBaseProps;


    public SuppressionGroup send(final TokenRestClient client) {
        String path = "/v3/asm/groups/{group_id}";
        Request.Builder builder = new Request.Builder(HttpMethod.PATCH, path)
                .addPathParam("group_id", groupId.toString());
        ...
        if (response == null) {
            throw new ApiConnectionException(
                    "PatchGroupsById creation failed: Unable to connect to server"
            );
        } else if (!TokenRestClient.SUCCESS.test(response.getStatusCode())) {

            int statusCode = response.getStatusCode();
        }
        return JsonUtil.fromJson(response.getStream(), SuppressionGroup.class);
    }
    ...
    private void body(final Request.Builder builder) {
        if (suppressionGroupRequestBaseProps != null) {
            builder.body(JsonUtil.toJson(suppressionGroupRequestBaseProps));
        }
    }
