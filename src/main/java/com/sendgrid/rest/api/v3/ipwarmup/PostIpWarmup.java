...
import com.sendgrid.rest.api.v3.ipwarmup.models.PostIpsWarmup404Response;
import com.sendgrid.rest.api.v3.ipwarmup.models.PostIpsWarmupRequest;
import com.sendgrid.util.JsonUtil;
import com.sendgrid.util.Matcher;
import lombok.RequiredArgsConstructor;

@RequiredArgsConstructor
public class PostIpWarmup {
    private final PostIpsWarmupRequest postIpsWarmupRequest;


    public List<IpWarmup200Inner> send(final TokenRestClient client) {
        ...
        if (response == null) {
            throw new ApiConnectionException(
                    "PostIpWarmup creation failed: Unable to connect to server"
            );
        } else if (!TokenRestClient.SUCCESS.test(response.getStatusCode())) {

            int statusCode = response.getStatusCode();
            if (Matcher.matches(Integer.toString(statusCode), "404")) {
                PostIpsWarmup404Response error = JsonUtil.fromJson(
                        response.getStream(),
                        PostIpsWarmup404Response.class
                );
                throw new ApiErrorResponse(statusCode, null, error, response.getHeaders());
            }

        }
        return JsonUtil.fromJson(response.getStream(), List & lt; IpWarmup200Inner & gt;.class);
    }


    private void body(final Request.Builder builder) {
        if (postIpsWarmupRequest != null) {
            builder.body(JsonUtil.toJson(postIpsWarmupRequest));
        }
    }
