...
import com.sendgrid.rest.api.v3.linkbranding.models.PostWhitelabelLinksLinkIdSubuserRequest;
import com.sendgrid.util.JsonUtil;
import lombok.RequiredArgsConstructor;

@RequiredArgsConstructor
public class PostLinkWithASubuser {
    private final Integer linkId;
    private final PostWhitelabelLinksLinkIdSubuserRequest postWhitelabelLinksLinkIdSubuserRequest;


    public LinkBranding200 send(final TokenRestClient client) {
        String path = "/v3/whitelabel/links/{link_id}/subuser";
        Request.Builder builder = new Request.Builder(HttpMethod.POST, path)
                .addPathParam("link_id", linkId.toString());
        body(builder);
        ...
        if (response == null) {
            throw new ApiConnectionException(
                    "PostLinkWithASubuser creation failed: Unable to connect to server"
            );
        } else if (!TokenRestClient.SUCCESS.test(response.getStatusCode())) {

            int statusCode = response.getStatusCode();
        }
        return JsonUtil.fromJson(response.getStream(), LinkBranding200.class);
    }


    private void body(final Request.Builder builder) {
        if (postWhitelabelLinksLinkIdSubuserRequest != null) {
            builder.body(JsonUtil.toJson(postWhitelabelLinksLinkIdSubuserRequest));
        }
    }
