...
import com.sendgrid.rest.api.v3.ips.models.GetIpsPoolsPoolName200Response;
import com.sendgrid.rest.api.v3.ips.models.GetIpsPoolsPoolName404Response;
import com.sendgrid.util.JsonUtil;
import com.sendgrid.util.Matcher;
import lombok.RequiredArgsConstructor;

@RequiredArgsConstructor
public class GetIpPoolById {
    private final String poolName;


    public GetIpsPoolsPoolName200Response send(final TokenRestClient client) {
        ...
        if (response == null) {
            throw new ApiConnectionException(
                    "GetIpPoolById creation failed: Unable to connect to server"
            );
        } else if (!TokenRestClient.SUCCESS.test(response.getStatusCode())) {

            int statusCode = response.getStatusCode();
            if (Matcher.matches(Integer.toString(statusCode), "404")) {
                GetIpsPoolsPoolName404Response error = JsonUtil.fromJson(
                        response.getStream(),
                        GetIpsPoolsPoolName404Response.class
                );
                throw new ApiErrorResponse(statusCode, null, error, response.getHeaders());
            }

        }
        return JsonUtil.fromJson(response.getStream(), GetIpsPoolsPoolName200Response.class);
    }
