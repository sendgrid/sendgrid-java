...
import com.sendgrid.rest.api.v3.user.models.GETUserProfileResponse;
...
public class GetProfile {
    ...
    public GETUserProfileResponse send(final TokenRestClient client) {
        ...
        if (response == null) {
            throw new ApiConnectionException(
                    "GetProfile creation failed: Unable to connect to server"
            );
        } else if (!TokenRestClient.SUCCESS.test(response.getStatusCode())) {

            int statusCode = response.getStatusCode();
        }
        return JsonUtil.fromJson(response.getStream(), GETUserProfileResponse.class);
    }
