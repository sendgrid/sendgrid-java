...
public class RefreshSegment extends ApiKeyBase {
    ...
    public ApiResponse<SegmentRefresh202> send(final ApiKeyRestClient client) {
        ...
        if (response == null) {
            throw new ApiConnectionException(
                "RefreshSegment creation failed: Unable to connect to server"
            );
        } else if (
    ...
    private void addBody(final Request request) {
        if (segmentRefreshRequest != null) {
            request.addBody(JsonUtil.toJson(segmentRefreshRequest));
        }
    }
}
