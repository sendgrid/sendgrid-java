...
import com.sendgrid.rest.api.v3.domainauthentication.models.PostWhitelabelDnsEmail400Response;
import com.sendgrid.rest.api.v3.domainauthentication.models.PostWhitelabelDnsEmailRequest;
import com.sendgrid.util.JsonUtil;
import com.sendgrid.util.Matcher;
import lombok.RequiredArgsConstructor;

@RequiredArgsConstructor
public class PostWhitelabelDnsEmail {
    private final PostWhitelabelDnsEmailRequest postWhitelabelDnsEmailRequest;
    ...
    public void send(final TokenRestClient client) {
        ...
        if (response == null) {
            throw new ApiConnectionException(
                    "PostWhitelabelDnsEmail creation failed: Unable to connect to server"
            );
        } else if (!TokenRestClient.SUCCESS.test(response.getStatusCode())) {

            int statusCode = response.getStatusCode();
            if (Matcher.matches(Integer.toString(statusCode), "400")) {
                PostWhitelabelDnsEmail400Response error = JsonUtil.fromJson(
                        response.getStream(),
                        PostWhitelabelDnsEmail400Response.class
                );
    ...
    private void body(final Request.Builder builder) {
        if (postWhitelabelDnsEmailRequest != null) {
            builder.body(JsonUtil.toJson(postWhitelabelDnsEmailRequest));
        }
    }
