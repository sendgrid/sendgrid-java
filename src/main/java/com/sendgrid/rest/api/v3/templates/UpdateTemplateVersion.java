...
public class UpdateTemplateVersion extends ApiKeyBase {
    ...
    public ApiResponse<TransactionalTemplateVersionOutput> send(
        ...
        if (response == null) {
            throw new ApiConnectionException(
                "UpdateTemplateVersion creation failed: Unable to connect to server"
            );
        } else if (
        ...
        return new ApiResponse(
            ...
            JsonUtil.fromJson(
                response.getStream(),
                TransactionalTemplateVersionOutput.class
            ),
            response.getHeaders()
        );
    }
    ...
    private void addPathParams(Request request) {
        ...
        if (versionId != null) {
            request.addPathParam("version_id", versionId.toString());
        }
    }
    ...
    private void addBody(final Request request) {
        if (transactionalTemplateVersionCreate != null) {
            request.addBody(
                JsonUtil.toJson(transactionalTemplateVersionCreate)
            );
        }
    }
}
