...
public class GetTemplateMailSetting {
    private String onBehalfOf;


    public MailSettingsTemplate200 send(final TokenRestClient client) {
        ...
        if (response == null) {
            throw new ApiConnectionException(
                    "GetTemplateMailSetting creation failed: Unable to connect to server"
            );
        } else if (!TokenRestClient.SUCCESS.test(response.getStatusCode())) {

            int statusCode = response.getStatusCode();
        }
        return JsonUtil.fromJson(response.getStream(), MailSettingsTemplate200.class);
    }
