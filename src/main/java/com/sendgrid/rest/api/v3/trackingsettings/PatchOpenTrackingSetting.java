...
import com.sendgrid.rest.api.v3.trackingsettings.models.GetTrackingSettingsOpen200Response;
import com.sendgrid.rest.api.v3.trackingsettings.models.PatchTrackingSettingsOpenRequest;
import com.sendgrid.util.JsonUtil;
import lombok.RequiredArgsConstructor;

@RequiredArgsConstructor
public class PatchOpenTrackingSetting {
    private String onBehalfOf;
    private final PatchTrackingSettingsOpenRequest patchTrackingSettingsOpenRequest;


    public GetTrackingSettingsOpen200Response send(final TokenRestClient client) {
        String path = "/v3/tracking_settings/open";
        ...
        if (response == null) {
            throw new ApiConnectionException(
                    "PatchOpenTrackingSetting creation failed: Unable to connect to server"
            );
        } else if (!TokenRestClient.SUCCESS.test(response.getStatusCode())) {

            int statusCode = response.getStatusCode();
        }
        return JsonUtil.fromJson(response.getStream(), GetTrackingSettingsOpen200Response.class);
    }
    ...
    private void body(final Request.Builder builder) {
        if (patchTrackingSettingsOpenRequest != null) {
            builder.body(JsonUtil.toJson(patchTrackingSettingsOpenRequest));
        }
    }
