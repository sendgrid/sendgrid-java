...
import com.sendgrid.rest.api.v3.user.models.PutUserEmail200Response;
import com.sendgrid.rest.api.v3.user.models.PutUserEmailRequest;
...
public class PutEmail {
    ...
    private final PutUserEmailRequest putUserEmailRequest;


    public PutUserEmail200Response send(final TokenRestClient client) {
        ...
        if (response == null) {
            throw new ApiConnectionException(
                    "PutEmail creation failed: Unable to connect to server"
            );
        } else if (!TokenRestClient.SUCCESS.test(response.getStatusCode())) {

            int statusCode = response.getStatusCode();
        }
        return JsonUtil.fromJson(response.getStream(), PutUserEmail200Response.class);
    }
    ...
    private void body(final Request.Builder builder) {
        if (putUserEmailRequest != null) {
            builder.body(JsonUtil.toJson(putUserEmailRequest));
        }
    }
