...
import com.sendgrid.rest.api.v3.templates.models.PatchTemplatesTemplateIdRequest;
import com.sendgrid.rest.api.v3.templates.models.TransactionalTemplate;
import com.sendgrid.util.JsonUtil;
import lombok.RequiredArgsConstructor;

@RequiredArgsConstructor
public class PatchTemplatesById {
    private final String templateId;
    private String onBehalfOf;
    private final PatchTemplatesTemplateIdRequest patchTemplatesTemplateIdRequest;


    public TransactionalTemplate send(final TokenRestClient client) {
        ...
        if (response == null) {
            throw new ApiConnectionException(
                    "PatchTemplatesById creation failed: Unable to connect to server"
            );
        } else if (!TokenRestClient.SUCCESS.test(response.getStatusCode())) {

            int statusCode = response.getStatusCode();
        }
        return JsonUtil.fromJson(response.getStream(), TransactionalTemplate.class);
    }
    ...
    private void body(final Request.Builder builder) {
        if (patchTemplatesTemplateIdRequest != null) {
            builder.body(JsonUtil.toJson(patchTemplatesTemplateIdRequest));
        }
    }
