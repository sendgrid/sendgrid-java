...
public class PutAccountOffering {
    private final String accountID;
    private final OfferingsToAdd offeringsToAdd;


    public AccountProvisioningOfferingList send(final TokenRestClient client) {
        String path = "/v3/partners/accounts/{accountID}/offerings";
        Request.Builder builder = new Request.Builder(HttpMethod.PUT, path)
                .addPathParam("accountID", accountID.toString());
        body(builder);
        ...
        if (response == null) {
            throw new ApiConnectionException(
                    "PutAccountOffering creation failed: Unable to connect to server"
            );
        } else if (!TokenRestClient.SUCCESS.test(response.getStatusCode())) {

            int statusCode = response.getStatusCode();
        }
        return JsonUtil.fromJson(response.getStream(), AccountProvisioningOfferingList.class);
    }


    private void body(final Request.Builder builder) {
        if (offeringsToAdd != null) {
            builder.body(JsonUtil.toJson(offeringsToAdd));
        }
    }
