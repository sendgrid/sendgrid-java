...
import com.sendgrid.rest.api.v3.suppressions.models.PostAsmGroupsGroupIdSuppressions201Response;
import com.sendgrid.rest.api.v3.suppressions.models.SuppressionsRequest;
import com.sendgrid.util.JsonUtil;
import lombok.RequiredArgsConstructor;

@RequiredArgsConstructor
public class PostGroupSuppression {
    private final String groupId;
    private String onBehalfOf;
    private final SuppressionsRequest suppressionsRequest;


    public PostAsmGroupsGroupIdSuppressions201Response send(final TokenRestClient client) {
        ...
        if (response == null) {
            throw new ApiConnectionException(
                    "PostGroupSuppression creation failed: Unable to connect to server"
            );
        } else if (!TokenRestClient.SUCCESS.test(response.getStatusCode())) {

            int statusCode = response.getStatusCode();
        }
        return JsonUtil.fromJson(response.getStream(), PostAsmGroupsGroupIdSuppressions201Response.class);
    }
    ...
    private void body(final Request.Builder builder) {
        if (suppressionsRequest != null) {
            builder.body(JsonUtil.toJson(suppressionsRequest));
        }
    }
