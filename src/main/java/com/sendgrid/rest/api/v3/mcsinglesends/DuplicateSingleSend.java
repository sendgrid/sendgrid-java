...
import com.sendgrid.rest.api.v3.mcsinglesends.models.DuplicateSingleSendRequest;
...
public class DuplicateSingleSend extends ApiKeyBase {

    private final String id;

    @Setter
    private DuplicateSingleSendRequest duplicateSingleSendRequest;

    public ApiResponse<SinglesendResponse> send(final ApiKeyRestClient client) {
        ...
        if (response == null) {
            throw new ApiConnectionException(
                "DuplicateSingleSend creation failed: Unable to connect to server"
            );
        } else if (
        ...
        return new ApiResponse(
            statusCode,
            JsonUtil.fromJson(response.getStream(), SinglesendResponse.class),
            response.getHeaders()
        );
    }
    ...
    private void addBody(final Request request) {
        if (duplicateSingleSendRequest != null) {
            request.addBody(JsonUtil.toJson(duplicateSingleSendRequest));
        }
    }
}
