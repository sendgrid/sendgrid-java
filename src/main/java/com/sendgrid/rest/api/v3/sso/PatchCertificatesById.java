...
import com.sendgrid.rest.api.v3.sso.models.PatchSsoCertificatesCertIdRequest;
...
public class PatchCertificatesById {
    private final String certId;
    private final PatchSsoCertificatesCertIdRequest patchSsoCertificatesCertIdRequest;


    public SsoCertificateBody send(final TokenRestClient client) {
        String path = "/v3/sso/certificates/{cert_id}";
        Request.Builder builder = new Request.Builder(HttpMethod.PATCH, path)
                .addPathParam("cert_id", certId.toString());
        body(builder);
        ...
        if (response == null) {
            throw new ApiConnectionException(
                    "PatchCertificatesById creation failed: Unable to connect to server"
            );
        ...
        return JsonUtil.fromJson(response.getStream(), SsoCertificateBody.class);
    }


    private void body(final Request.Builder builder) {
        if (patchSsoCertificatesCertIdRequest != null) {
            builder.body(JsonUtil.toJson(patchSsoCertificatesCertIdRequest));
        }
    }
