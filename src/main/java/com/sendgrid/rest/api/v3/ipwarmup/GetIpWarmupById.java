...
import com.sendgrid.rest.api.v3.ipwarmup.models.GetIpsWarmupIpAddress404Response;
    ...
    public List<IpWarmup200Inner> send(final TokenRestClient client) {
        ...
        if (response == null) {
            throw new ApiConnectionException(
                    "GetIpWarmupById creation failed: Unable to connect to server"
            );
        } else if (!TokenRestClient.SUCCESS.test(response.getStatusCode())) {

            int statusCode = response.getStatusCode();
            if (Matcher.matches(Integer.toString(statusCode), "404")) {
                GetIpsWarmupIpAddress404Response error = JsonUtil.fromJson(
                        response.getStream(),
                        GetIpsWarmupIpAddress404Response.class
                );
