...
import com.sendgrid.rest.api.v3.suppressions.models.SpamReportsResponseInner;
import com.sendgrid.util.JsonUtil;
import lombok.RequiredArgsConstructor;

@RequiredArgsConstructor
public class GetSuppressionSpamReportById {
    private final String email;
    private String onBehalfOf;


    public List<SpamReportsResponseInner> send(final TokenRestClient client) {
        String path = "/v3/suppression/spam_reports/{email}";
        ...
        if (response == null) {
            throw new ApiConnectionException(
                    "GetSuppressionSpamReportById creation failed: Unable to connect to server"
            );
        } else if (!TokenRestClient.SUCCESS.test(response.getStatusCode())) {

            int statusCode = response.getStatusCode();
        }
        return JsonUtil.fromJson(response.getStream(), List & lt; SpamReportsResponseInner & gt;.class);
    }
