...
import com.sendgrid.rest.api.v3.domainauthentication.models.PostWhitelabelDomainsRequest;
import com.sendgrid.util.JsonUtil;
import lombok.RequiredArgsConstructor;

@RequiredArgsConstructor
public class PostWhitelabelDomain {
    private String onBehalfOf;
    private final PostWhitelabelDomainsRequest postWhitelabelDomainsRequest;


    public AuthenticatedDomain send(final TokenRestClient client) {
        ...
        if (response == null) {
            throw new ApiConnectionException(
                    "PostWhitelabelDomain creation failed: Unable to connect to server"
            );
        } else if (!TokenRestClient.SUCCESS.test(response.getStatusCode())) {

            int statusCode = response.getStatusCode();
        }
        return JsonUtil.fromJson(response.getStream(), AuthenticatedDomain.class);
    }
    ...
    private void body(final Request.Builder builder) {
        if (postWhitelabelDomainsRequest != null) {
            builder.body(JsonUtil.toJson(postWhitelabelDomainsRequest));
        }
    }
