...
import com.sendgrid.rest.api.v3.mccontacts.models.GetMarketingContacts400Response;
import com.sendgrid.rest.api.v3.mccontacts.models.PostMarketingContactsExports202Response;
import com.sendgrid.rest.api.v3.mccontacts.models.PostMarketingContactsExportsRequest;
...
public class PostMarketingContactExport {
    private final PostMarketingContactsExportsRequest postMarketingContactsExportsRequest;


    public PostMarketingContactsExports202Response send(final TokenRestClient client) {
        ...
        if (response == null) {
            throw new ApiConnectionException(
                    "PostMarketingContactExport creation failed: Unable to connect to server"
            );
        } else if (!TokenRestClient.SUCCESS.test(response.getStatusCode())) {

            int statusCode = response.getStatusCode();
            if (Matcher.matches(Integer.toString(statusCode), "400")) {
                GetMarketingContacts400Response error = JsonUtil.fromJson(
                        response.getStream(),
                        GetMarketingContacts400Response.class
                );
        ...
        return JsonUtil.fromJson(response.getStream(), PostMarketingContactsExports202Response.class);
    }


    private void body(final Request.Builder builder) {
        if (postMarketingContactsExportsRequest != null) {
            builder.body(JsonUtil.toJson(postMarketingContactsExportsRequest));
        }
    }
