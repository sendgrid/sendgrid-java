...
public class GetSuppressionBlockById {
    private final String email;
    private String onBehalfOf;


    public List<BlocksResponseInner> send(final TokenRestClient client) {
        ...
        if (response == null) {
            throw new ApiConnectionException(
                    "GetSuppressionBlockById creation failed: Unable to connect to server"
            );
        } else if (!TokenRestClient.SUCCESS.test(response.getStatusCode())) {

            int statusCode = response.getStatusCode();
        }
        return JsonUtil.fromJson(response.getStream(), List & lt; BlocksResponseInner & gt;.class);
    }
