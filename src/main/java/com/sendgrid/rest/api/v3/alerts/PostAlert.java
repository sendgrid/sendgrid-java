...
import com.sendgrid.rest.api.v3.alerts.models.PostAlerts201Response;
import com.sendgrid.rest.api.v3.alerts.models.PostAlerts400Response;
import com.sendgrid.rest.api.v3.alerts.models.PostAlertsRequest;
...
public class PostAlert {
    ...
    private final PostAlertsRequest postAlertsRequest;


    public PostAlerts201Response send(final TokenRestClient client) {
        ...
        if (response == null) {
            throw new ApiConnectionException(
                    "PostAlert creation failed: Unable to connect to server"
            );
        } else if (!TokenRestClient.SUCCESS.test(response.getStatusCode())) {

            int statusCode = response.getStatusCode();
            if (Matcher.matches(Integer.toString(statusCode), "400")) {
                PostAlerts400Response error = JsonUtil.fromJson(
                        response.getStream(),
                        PostAlerts400Response.class
                );
                throw new ApiErrorResponse(statusCode, null, error, response.getHeaders());
            }

        }
        return JsonUtil.fromJson(response.getStream(), PostAlerts201Response.class);
    }
    ...
    private void body(final Request.Builder builder) {
        if (postAlertsRequest != null) {
            builder.body(JsonUtil.toJson(postAlertsRequest));
        }
    }
