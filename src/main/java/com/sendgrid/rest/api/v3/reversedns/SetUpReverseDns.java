...
import com.sendgrid.rest.api.v3.reversedns.models.SetUpReverseDnsRequest;
import com.sendgrid.util.JsonUtil;
import lombok.RequiredArgsConstructor;
import lombok.Setter;

@RequiredArgsConstructor
public class SetUpReverseDns extends ApiKeyBase {

    @Setter
    private String onBehalfOf;

    @Setter
    private SetUpReverseDnsRequest setUpReverseDnsRequest;

    public ApiResponse<ReverseDns> send(final ApiKeyRestClient client) {
        ...
        if (response == null) {
            throw new ApiConnectionException(
                "SetUpReverseDns creation failed: Unable to connect to server"
            );
        } else if (
        ...
        return new ApiResponse(
            statusCode,
            JsonUtil.fromJson(response.getStream(), ReverseDns.class),
            response.getHeaders()
        );
    }
    ...
    private void addBody(final Request request) {
        if (setUpReverseDnsRequest != null) {
            request.addBody(JsonUtil.toJson(setUpReverseDnsRequest));
        }
    }
}
