...
import com.sendgrid.rest.api.v3.ips.models.DeleteIpsPoolsPoolNameIpsIp404Response;
import com.sendgrid.util.JsonUtil;
import com.sendgrid.util.Matcher;
import lombok.RequiredArgsConstructor;

@RequiredArgsConstructor
public class DeleteIpPoolIpById {
    ...
    public Object send(final TokenRestClient client) {
        ...
        if (response == null) {
            throw new ApiConnectionException(
                    "DeleteIpPoolIpById creation failed: Unable to connect to server"
            );
        } else if (!TokenRestClient.SUCCESS.test(response.getStatusCode())) {

            int statusCode = response.getStatusCode();
            if (Matcher.matches(Integer.toString(statusCode), "404")) {
                DeleteIpsPoolsPoolNameIpsIp404Response error = JsonUtil.fromJson(
                        response.getStream(),
                        DeleteIpsPoolsPoolNameIpsIp404Response.class
                );
