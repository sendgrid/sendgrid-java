...
public class PatchFooterMailSetting {
    private String onBehalfOf;
    private final MailSettingsFooter mailSettingsFooter;


    public MailSettingsFooter send(final TokenRestClient client) {
        ...
        if (response == null) {
            throw new ApiConnectionException(
                    "PatchFooterMailSetting creation failed: Unable to connect to server"
            );
        } else if (!TokenRestClient.SUCCESS.test(response.getStatusCode())) {

            int statusCode = response.getStatusCode();
        }
        return JsonUtil.fromJson(response.getStream(), MailSettingsFooter.class);
    }
    ...
    private void body(final Request.Builder builder) {
        if (mailSettingsFooter != null) {
            builder.body(JsonUtil.toJson(mailSettingsFooter));
        }
    }
