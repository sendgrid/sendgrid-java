...
import com.sendgrid.rest.api.v3.domainauthentication.models.PostWhitelabelDomainsDomainIdSubuserRequest;
import com.sendgrid.util.JsonUtil;
import lombok.RequiredArgsConstructor;

@RequiredArgsConstructor
public class PostWhitelabelDomainsSubuserAssociation {
    private final Integer domainId;
    private final PostWhitelabelDomainsDomainIdSubuserRequest postWhitelabelDomainsDomainIdSubuserRequest;


    public AuthenticatedDomainSpf send(final TokenRestClient client) {
        String path = "/v3/whitelabel/domains/{domain_id}/subuser";
        Request.Builder builder = new Request.Builder(HttpMethod.POST, path)
                .addPathParam("domain_id", domainId.toString());
        body(builder);
        ...
        if (response == null) {
            throw new ApiConnectionException(
                    "PostWhitelabelDomainsSubuserAssociation creation failed: Unable to connect to server"
            );
        } else if (!TokenRestClient.SUCCESS.test(response.getStatusCode())) {

            int statusCode = response.getStatusCode();
        }
        return JsonUtil.fromJson(response.getStream(), AuthenticatedDomainSpf.class);
    }


    private void body(final Request.Builder builder) {
        if (postWhitelabelDomainsDomainIdSubuserRequest != null) {
            builder.body(JsonUtil.toJson(postWhitelabelDomainsDomainIdSubuserRequest));
        }
    }
