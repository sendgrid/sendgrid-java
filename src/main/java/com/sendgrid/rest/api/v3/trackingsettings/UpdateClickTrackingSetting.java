...
import com.sendgrid.rest.api.v3.trackingsettings.models.ClickTracking;
import com.sendgrid.rest.api.v3.trackingsettings.models.UpdateClickTrackingSettingRequest;
import com.sendgrid.util.JsonUtil;
import lombok.RequiredArgsConstructor;
import lombok.Setter;

@RequiredArgsConstructor
public class UpdateClickTrackingSetting extends ApiKeyBase {

    @Setter
    private String onBehalfOf;

    @Setter
    private UpdateClickTrackingSettingRequest updateClickTrackingSettingRequest;

    public ApiResponse<ClickTracking> send(final ApiKeyRestClient client) {
        String path = "/v3/tracking_settings/click";
        ...
        if (response == null) {
            throw new ApiConnectionException(
                "UpdateClickTrackingSetting creation failed: Unable to connect to server"
            );
        } else if (
        ...
        return new ApiResponse(
            statusCode,
            JsonUtil.fromJson(response.getStream(), ClickTracking.class),
            response.getHeaders()
        );
    }
    ...
    private void addBody(final Request request) {
        if (updateClickTrackingSettingRequest != null) {
            request.addBody(JsonUtil.toJson(updateClickTrackingSettingRequest));
        }
    }
}
