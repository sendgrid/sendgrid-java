...
import com.sendgrid.rest.api.v3.suppressions.models.PostAsmSuppressionsGlobal201Response;
import com.sendgrid.rest.api.v3.suppressions.models.SuppressionsRequest;
import com.sendgrid.util.JsonUtil;
import lombok.RequiredArgsConstructor;

@RequiredArgsConstructor
public class PostGroupSuppressionGlobal {
    private String onBehalfOf;
    private final SuppressionsRequest suppressionsRequest;


    public PostAsmSuppressionsGlobal201Response send(final TokenRestClient client) {
        String path = "/v3/asm/suppressions/global";
        ...
        if (response == null) {
            throw new ApiConnectionException(
                    "PostGroupSuppressionGlobal creation failed: Unable to connect to server"
            );
        } else if (!TokenRestClient.SUCCESS.test(response.getStatusCode())) {

            int statusCode = response.getStatusCode();
        }
        return JsonUtil.fromJson(response.getStream(), PostAsmSuppressionsGlobal201Response.class);
    }
